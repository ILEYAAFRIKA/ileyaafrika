      ...
      <div className="space-y-3">
        <div className="flex items-baseline justify-between border-b border-[#1B4332]/10 pb-3.5">
          <div>
            ...
            {/* Average Star Rating & Total Reviews Count near price */}
            {totalReviewCount !== undefined && totalReviewCount > 0 && averageRating !== null && averageRating !== undefined ? (
              <div className="flex items-center gap-1.5 mt-1.5 text-xs font-semibold text-[#14231C]">
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200/80">
                  <Star className="w-3.5 h-3.5 fill-[#E8A33D] text-[#E8A33D]" />
                  <span className="font-bold">{averageRating.toFixed(1)}</span>
                </div>
                <span className="text-[#6B756F] text-[11px]">
                  ({totalReviewCount} {totalReviewCount === 1 ? 'review' : 'reviews'})
                </span>
              </div>
            ) : (
              <div className="flex items-center gap-1 mt-1 text-[11px] text-[#6B756F]">
                <Star className="w-3 h-3 text-gray-400" />
                <span>New Verified Stay • No reviews yet</span>
              </div>
            )}
          </div>
